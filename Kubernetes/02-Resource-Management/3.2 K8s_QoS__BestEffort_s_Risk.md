...
> 
> 
> There is **no `resources` section** → **no `requests`**, **no `limits`** for CPU or memory.
> 
...
| --- | --- | --- | --- |
| **Guaranteed** | `requests == limits` for **all** containers & **all** resources (CPU + memory) | 🔝 Highest | Never evicted due to resource pressure (if node has capacity) |
| **Burstable** | Some resources have `requests` ≠ `limits`, or only `requests`/`limits` set for **some** resources | 🟡 Medium | Evicted **after** BestEffort, **before** Guaranteed |
| **BestEffort** | ❌ **No `requests` or `limits`** for **any** container | 🔻 Lowest | **First to be evicted** under memory pressure! |

> 💡 BestEffort = "Use whatever is free — but we make no promises."
> 
...
- **No scheduling guarantees**: Scheduler doesn’t reserve resources for it.
...
### 🧪 Lab: Deploy BestEffort Pod & Verify QoS
...
# 4. Describe Pod → look for "QoS Class"
...
### 🔍 Compare with Other QoS Classes (Preview)

| Pod Type | `resources` Config | QoS Class |
| --- | --- | --- |
| This Pod | ❌ None | **BestEffort** |
...
A: **No.** The **lowest** QoS among containers determines the **Pod’s QoS**.

Example: 1 container = Guaranteed, 1 = BestEffort → **entire Pod = BestEffort**.

**Q: Does BestEffort mean no CPU/memory usage?**

A: **No!** It can use **all available resources** — but gets **no protection** when resources are scarce.
...
✅ **BestEffort** = **no resource requests/limits** defined.

⚠️ **Lowest priority** — first to be **killed under memory pressure**.

🔍 Verify with: `kubectl describe pod <name> | grep "QoS Class"`

🚫 **Avoid in production** — always set at least **`requests`**.