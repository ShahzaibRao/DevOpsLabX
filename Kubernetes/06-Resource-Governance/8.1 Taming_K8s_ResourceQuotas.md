...
metadata:
  name: pod-quota
  namespace: learning    # ← Applied to this namespace only
spec:
  hard:
    pods: "4"            # ← Max 4 Pods allowed in "learning" namespace
...
> 
> 
> This quota **prevents users/teams** from flooding a namespace with unlimited Pods — critical for **cost control and stability**.
> 
...
| **`hard.pods: "4"`** | Hard limit — **no more than 4 Pods** (running + pending) |
| **Enforcement** | Happens at **admission time** (before Pod is created) |

> 🎯 What counts toward pods quota?
> 
...
> 
> 
> Quotas apply to **all Pods** in the namespace — even if created by controllers (Deployments, etc.).
> 

---

### 🧪 k3s Lab: Enforce Pod Quota
...
### 🔧 Step 2: Deploy Pods Up to Quota Limit
...
> 
> 
> **"exceeded quota"** → Kubernetes **blocks creation** at admission time.
> 
...
A: ✅ **Yes!** If you deploy a Deployment with `replicas: 10` but quota=4:
...
A: Quota usage **decreases immediately** → you can create new Pods.

**Q: Can I have multiple ResourceQuota objects in one namespace?**

A: ❌ **No!** Only **one ResourceQuota** per namespace (but it can have many rules).

**Q: Does quota apply to system Pods (kube-system)?**

A: ❌ **No!** Quotas only apply to **user namespaces** (not `kube-system`, `default`, etc.).
...
✅ **`pods: "4"`** = max 4 Pods (any type) in namespace

🔍 In k3s: Test by **exceeding quota** → watch **"forbidden" error**

🛠️ Always set quotas in **multi-team clusters**